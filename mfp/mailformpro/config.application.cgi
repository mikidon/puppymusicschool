...
$config{'ThanksPage'} = '../../application-thanks';

## 設置者に届くメールの件名
$config{'subject'} = 'PUPPY MUSIC SCHOOL：体験レッスンお申込みフォームから';

####ここから通知メール本文####
$_TEXT{'posted'} = <<'__posted_body__';

PUPPY MUSIC SCHOOL：体験レッスンお申込みフォームより以下のメールを受付ました。
...
## 自動返信メールの件名
$config{"ReturnSubject"} = '【 PUPPY MUSIC SCHOOL 】 お申込みありがとうございます';
